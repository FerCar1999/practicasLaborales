...
function dataTable() {
    ...
    table = $('#table-profesion').DataTable({
        destroy: true,
        ...
        },
        {
            ...
            defaultContent: "<a href='#updateProfesion' class='update btn-small blue darken-1 waves-effect waves-ligth modal-trigger'>Modificar</a>"
                + "  <a href='#deleteProfesion' class='delete btn-small red darken-1 waves-effect waves-ligth modal-trigger'>Eliminar</a>"
        }],
    ...
    getDataToUpdate("#table-profesion tbody", table);
    // Llamamos al metodo para obtener el id del registro, para eliminar
    getIdToDelete("#table-profesion tbody", table);
}
// Funcion para obtener datos para modificar
function getDataToUpdate(tbody, table) {
    $('tbody').on("click", "a.update", function () {
        var data = table.row($(this).parents("tr")).data();
        $("#nombProfUpda").next("label").addClass("active");
        var codi_prof = $("#codiProfUpda").val(data.codi_prof),
            nomb_prof = $("#nombProfUpda").val(data.nomb_prof);
    });
}
...
function getIdToDelete(tobyd, table) {
    $('tbody').on("click", "a.delete", function () {
        var data = table.row($(this).parents("tr")).data();
        var codi_prof = $("#codiProfDele").val(data.codi_prof)
    });
}
    ...
    var datos = $('#frmAdd').serialize();
            ...
            success: function (data) {
                ...
                if (resp >= 0) {
                    ...
                    M.toast({ html: "Profesion agregada con exito", classes: 'rounded' });
    ...
    $.ajax({
            ...
            success: function (data) {
                ...
                if (resp >= 0) {
                    ...
                    M.toast({ html: "Profesion modificada con exito", classes: 'rounded' });
                    // Recargando la tabla
                    table.ajax.reload();
                    //Mostrando el footer del modal
                    $('.modal-footer').show();
...
function remove() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (resp >= 0) {
                M.toast({ html: "Profesion eliminada con exito", classes: 'rounded' });
                // Recargando la tabla
                table.ajax.reload();
                //mostrando el footer del modal
                $('.modal-footer').show();
